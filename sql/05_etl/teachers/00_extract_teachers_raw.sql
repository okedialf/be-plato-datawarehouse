...
--
-- FIX: Added DISTINCT ON (t.person_id) to handle teachers with multiple
-- school_employees rows — picks the most recently updated employee record.
-- =============================================================================
...
SELECT DISTINCT ON (t.person_id)
    ...
    MAX(CASE WHEN UPPER(idt.name) LIKE '%NATIONAL%'
             ...
             THEN id_docs.document_id END)
        OVER (PARTITION BY t.person_id)                 AS nin,
    MAX(CASE WHEN UPPER(idt.name) LIKE '%PASSPORT%'
             THEN id_docs.document_id END)
        OVER (PARTITION BY t.person_id)                 AS passport_no,
...
ORDER BY t.person_id, se.date_updated DESC NULLS LAST;
